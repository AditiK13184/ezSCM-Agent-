...
test.describe('Edit Item', () => {
  const testData = getTestData('EditItem', ['Reorder Level', 'Unit Price']);

  for (const [index, row] of testData.entries()) {
    test(`TC-INV-023 - Verify editing an existing inventory item details - Row ${index + 1}`, async ({ page }) => {
      ...
      await page.locator('input[name="reorder_level"]').fill(row['Reorder Level']);
      await page.locator('input[name="unit_price"]').fill(row['Unit Price']);
